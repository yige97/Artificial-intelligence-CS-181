...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        for each in eliminationOrder:
            factors, joinedFactors = joinFactorsByVariable(factors, each)
            if len(joinedFactors.unconditionedVariables()) > 1:
                factors.append(eliminate(joinedFactors, each))

        allJointFactors = joinFactors(factors)
        return normalize(allJointFactors)
