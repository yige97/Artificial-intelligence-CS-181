...
def fillObsCPT(bayesNet, gameState):
    ...
    for House in gameState.getPossibleHouses():
        ...
        for Wall in gameState.getHouseWalls(House):
            ...
            wallCPT = bn.Factor([Wall], HOUSE_VARS, bayesNet.variableDomainsDict())
            for assignment in wallCPT.getAllPossibleAssignmentDicts():
                HouseVal = HouseVals[house_number]
                foodAssignment = assignment[FOOD_HOUSE_VAR]
                ghostAssignment = assignment[GHOST_HOUSE_VAR]
                curAssignmnt = assignment[Wall]
                prob = 0
                if foodAssignment == HouseVal:
                    if curAssignmnt == RED_OBS_VAL:
                        prob = PROB_FOOD_RED
                    elif curAssignmnt == BLUE_OBS_VAL:
                        prob = 1 - PROB_FOOD_RED
                elif ghostAssignment == HouseVal:
                    if curAssignmnt == RED_OBS_VAL:
                        prob = PROB_GHOST_RED
                    elif curAssignmnt == BLUE_OBS_VAL:
                        prob = 1 - PROB_GHOST_RED
                elif curAssignmnt == NO_OBS_VAL:
                    prob = 1
                wallCPT.setProbability(assignment, prob)
            bayesNet.setCPT(Wall, wallCPT)
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    "*** YOUR CODE HERE ***"
    Most = [0, 0]
    elimination = inference.inferenceByVariableElimination(bayesNet,FOOD_HOUSE_VAR, evidence, eliminationOrder)
    for each in elimination.getAllPossibleAssignmentDicts():
        probability = elimination.getProbability(each)
        if probability > Most[0]:
            Most = [probability, each]
    return Most[1]
